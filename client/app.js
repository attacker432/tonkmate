...
class Canvas {
  ...
  // =====================================================
                                            // Changing grid opacity.
                                            // =====================================================
                                            if (trimmedMessage.toLowerCase().startsWith('/gridopacity')){
                                                const args = trimmedMessage.split(' ');
                                                if (args.length === 2){
                                                    let opacity = parseFloat(args[1]);
                                                    if (!isNaN(opacity)){
                                                        if (opacity < 0) { opacity = 0; }
                                                        if (opacity > 1) { opacity = 1; }
                                                        //if (opacity >= 0 && opacity <= 1)
                                                        {
                                                            global.gridOpacity = opacity;                                                            
                                                            localStorage.setItem('gridOpacity', opacity);
                                                     }
                                                          } 
...
function startGame() {
    ...
    global.gridOpacity = localStorage.getItem('gridOpacity') || 1;
  // Get options
    ...
    { // Clear the background + draw grid
      ...
      ctx.globalAlpha = 1;
        // ========================================================================
            // Grid lines.
            // ========================================================================
            if (global.gridOpacity > 0 && global.gridOpacity <= 1){
                ctx.lineWidth = 1;
                ctx.strokeStyle = '#9A9A9A';
                ctx.globalAlpha = global.gridOpacity || 0.3;

                ctx.beginPath();
                let gridsize = 70 * ratio;
                for (let x = (global.screenWidth / 2 - px) % gridsize; x < global.screenWidth; x += gridsize) {
                    ctx.moveTo(x, 0);
                    ctx.lineTo(x, global.screenHeight);
                }
